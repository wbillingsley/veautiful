...
import com.wbillingsley.veautiful.svg.{DSvgContent, SVG, DSvgComponent}
...
/**
  ...
  * @tparam T
  */
case class FreeTile[T](tile:Tile[T]) extends DSvgComponent with OnScreen {
  
  val tileBoundary:DSvgContent = SVG.path(^.cls := "tile-path")
